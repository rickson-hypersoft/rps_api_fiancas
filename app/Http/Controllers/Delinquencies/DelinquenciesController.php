...
class DelinquenciesController extends Controller
{
    ...
    public function update(Request $request, int | string $id)
    {
        ...
        $requestData = [
            'TIPO_CONTA'          => isset($request->all()['tipo_conta']) ? mb_convert_encoding($request->all()['tipo_conta'], 'ISO-8859-1') : $delinquencies->TIPO_CONTA,
        ...
        if (! empty($request->input('novasContas'))) {
            foreach ($request->input('novasContas') as $tipo => $dados) {
                if ($tipo === 'conta') {
                    continue; // ignora a lista de tipos
                }
                ...
                $valores     = $dados['valor_original'] ?? [];
                $vencimentos = $dados['vencimento_original'] ?? [];

                foreach ($valores as $i => $valorOriginal) {
                    if (empty($valorOriginal)) {
                        continue; // ignora valores nulos/vazios
                    }

                    $vencimentoOriginal = $vencimentos[$i] ?? null;
    ...
    public function show(int | string $idImobiliaria, int | string $id)
    {
        ...
        $delinquencies = Delinquencies::with(['attachments', 'histories.user'])
    ...
    public function find(int | string $id)
    {
        ...
        return response()->json([
            'delinquencies' => new DelinquenciesResourceAttachment($delinquency),
        ]);
    }
    ...
    {
        $attachments = Attachment::where('ID_MOVI', '=', $id)->where('MOVI', '=', 'inadimplencias')->get();

        return response()->json(["anexos" => AttachamentResource::collection($attachments)]);
    }

    public function export(Request $request, string | int $idImobiliaria): \Symfony\Component\HttpFoundation\Response
    {
        $perPage = $request->get('per_page', 10); // quantidade por página, default 15

        $delinquencies = Delinquencies::with('propostal')->where('ID_IMOBILIARIA', $idImobiliaria)
            // Filtro por nome (via relação com contratos)
            ->when($request->filled('nome'), function ($query) use ($request): void {
                $query->whereHas('propostal', function ($q) use ($request): void {
                    $q->where('PESSOA_NOME', 'like', '%' . $request->nome . '%');
        ...
        return DelinquenciesAndPropostalResource::collection($delinquencies)
            ->response()
            ->setStatusCode(200);
    }
}